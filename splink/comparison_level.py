...
class ComparisonLevel:
    ...
    def _bayes_factor_sql(self):
        ...
        THEN cast({bayes_factor} as double precision)
        """
        return dedent(sql)
    ...
    def _tf_adjustment_sql(self):
        ...
        if self._comparison_vector_value == -1:
            sql = f"WHEN  {gamma_colname_value_is_this_level} then cast(1 as double precision)"
            ...
            if self._tf_minimum_u_value == 0.0:
                ...
                (CASE
                    WHEN {coalesce_l_r} >= {coalesce_r_l}
                    AND {coalesce_l_r} > cast({self._tf_minimum_u_value} as double precision)
                        THEN {coalesce_l_r}
                    WHEN {coalesce_r_l}  > cast({self._tf_minimum_u_value} as double precision)
                        THEN {coalesce_r_l}
                    ELSE cast({self._tf_minimum_u_value} as double precision)
                END)
                """
            ...
            WHEN  {gamma_colname_value_is_this_level} then
                ...
                THEN
                POW(
                    cast({u_prob_exact_match} as double precision) /{divisor_sql},
                    cast({self._tf_adjustment_weight} as double precision)
                )
                ELSE cast(1 as double precision)
                END)
            """
